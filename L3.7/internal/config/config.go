...
// Config описывает конфигурацию warehousecontrol-сервиса.
type Config struct {
...
// Auth содержит настройки JWT.
type Auth struct {
	JWTSecret string        `yaml:"jwt_secret" env-default:"warehousecontrol-local-secret"`
	TokenTTL  time.Duration `yaml:"token_ttl" env-default:"12h"`
}

// DB содержит настройки подключения к PostgreSQL.
type DB struct {
